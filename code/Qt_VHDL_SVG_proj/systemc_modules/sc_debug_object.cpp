...
debug_object::debug_object(sc_object *o) : obj(o), sc_kind(OBJ_KIND_OTHER)
{
...
bool debug_object::isModule()
{
    return (sc_kind == OBJ_KIND_MODULE);
}

bool debug_object::isAdaptor()
{
    return (sc_kind == OBJ_KIND_ADAPTOR);
}

bool debug_object::isPort()
{
    return (sc_kind == OBJ_KIND_PORT);
}

bool debug_object::isSignal()
{
    return (sc_kind == OBJ_KIND_SIGNAL);
}

bool debug_object::isHierarchy()
{
    return (sc_kind != OBJ_KIND_ADAPTOR) && (sc_kind != OBJ_KIND_OTHER);
}

bool debug_object::isTraceable()
{
    return (sc_kind == OBJ_KIND_CLOCK) || (sc_kind == OBJ_KIND_SIGNAL) || (sc_kind == OBJ_KIND_PORT);
}

std::string debug_object::name()
{
    if (nullptr != obj) return obj->name(); else return "";
}
