/**
 * @file sc_flexint.h
 * @brief FlexInt SystemC module
 *
 * This file contains the FlexInt SystemC module.
 *
 * The FlexInt module is a SystemC module that provides a flexible interface for
 * the reset and enable signals of a module. The reset and enable signals can be
 * active high or active low, and can be synchronous or asynchronous. The module
 * can be configured to use the reset and enable signals or not, and can be
 * configured to use the rising edge or falling edge of the clock signal.
 *
 * The module is a template class that can be instantiated with the following
 * parameters:
 * - CLKEDGE: boolean value that specifies the clock edge to use (true for rising
 *  edge, false for falling edge). Default value is true.
 *  - FLEXINT: flexinterface_t value that specifies the reset and enable signals
 *  configuration. Default value is RST_LOW_ENA_HIGH.
*/
...
typedef enum flexinterface_e
{
    NO_RST_NO_ENA       = 0x00,
    NO_RST_ENA_HIGH     = 0x01,
    NO_RST_ENA_LOW      = 0x02,
    RST_HIGH_NO_ENA     = 0x10,
    RST_HIGH_ENA_HIGH   = 0x11,
    RST_HIGH_ENA_LOW    = 0x12,
    RST_LOW_NO_ENA      = 0x20,
    RST_LOW_ENA_HIGH    = 0x21,
    RST_LOW_ENA_LOW     = 0x22
} flexinterface_t;

/** code used in sc_counter but commented out before here TOFIX
...
public:
    ...
    bool bResetAction()
    {
        return use_rst && (reset.read() == rst_level);
    }
    bool bEnableAction()
    {
        return !use_ena || (enable.read() == ena_level);
    }
